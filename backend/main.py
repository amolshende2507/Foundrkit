...
# ──────────────────────────────────────────────
# HEALTH CHECK  (visit /ping to confirm server is alive)
# ──────────────────────────────────────────────
@app.get("/ping")
def ping():
    return {"status": "alive"}
...
# ✅ SINGLE definition of LogoGenerationRequest
class LogoGenerationRequest(BaseModel):
    prompt: str
...
# ✅ SINGLE /branding/generate-image route (duplicate removed)
@app.post("/branding/generate-image")
def generate_image_logo(request: LogoGenerationRequest):
    print(f"DEBUG: Original User Input: {request.prompt}")
    ...
    try:
        bridge_prompt = f"""
Act as an expert Prompt Engineer for Stable Diffusion XL.
The user wants a professional startup logo for: "{request.prompt}"
Requirements:
- Minimalist flat vector logo
- Single centered symbol
- Pure white background
...
"""
    ...
    negative_prompt = "text, letters, typography, words, mockup, scene, background environment, photorealistic, 3d render, shadow, gradient, watermark, low quality, blurry"
    ...
    try:
        ...
        if response.status_code != 200:
            if "loading" in response.text.lower():
                raise HTTPException(status_code=503, detail="Logo model is warming up. Try again in 20 seconds.")
            raise HTTPException(status_code=500, detail=f"HuggingFace Error: {response.text}")

        base64_image = base64.b64encode(response.content).decode("utf-8")
        return {"image_url": f"data:image/png;base64,{base64_image}", "type": "logo"}
...
""".strip()
        ...
        prompt = f"""
You are a venture capitalist evaluating an early-stage startup.
Respond using ONLY the structure below. No emojis, no markdown, no extra commentary.
...
Draft Email: {data.get('draft')}
""".strip()

    elif tool_id == "eli5":
        prompt = f"""
Explain the following concept like I am 5 years old.
One short paragraph only. Simple language. No jargon, no emojis, no markdown.
...
Label each section clearly: Strengths, Weaknesses, Opportunities, Threats.
