...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const NavLink = ({ item }: { item: any }) => {
    ...
    return (
      ...
      >
        ...
        <span className={`${isCollapsed ? "md:hidden" : "block"} truncate`}>{item.name}</span>
      </Link>
    );
  };
  ...
  return (
    <div className="flex min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      >
        {/* LOGO */}
        <div className={`p-6 flex items-center ${isCollapsed ? "justify-center" : "gap-3"} border-b border-slate-100 dark:border-slate-800`}>
          ...
          {!isCollapsed && (
            <div>
              <h1 className="font-bold text-slate-900 dark:text-white">FoundrKit</h1>
              <p className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold">Startup OS</p>
            </div>
          )}
        </div>
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-800">
          <div className={`flex items-center ${isCollapsed ? "flex-col gap-2" : "gap-1"}`}>
            <Button
              ...
              className={`text-slate-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 ${isCollapsed ? "w-10" : "flex-1 justify-start gap-2"}`}
        ...
        <header className="md:hidden h-16 sticky top-0 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-4">
          ...
          <Sheet open={isOpen} onOpenChange={setIsOpen}>
            ...
            <SheetContent side="left" className="w-72 flex flex-col p-0">
                ...
                {/* MOBILE FOOTER WITH LOGOUT AND MODE TOGGLE */}
