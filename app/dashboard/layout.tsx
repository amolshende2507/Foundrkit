...
import Link from "next/link";
import { usePathname } from "next/navigation";
import { LayoutDashboard, FileText, Settings, LogOut, MessageSquare, Users, Mail, CheckSquare, Palette } from "lucide-react";
import { supabase } from "@/lib/supabase";
import { useRouter } from "next/navigation";
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 hidden md:flex flex-col h-full">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          {navItems.map((item) => {
            const Icon = item.icon;
            // Check if active (handles sub-routes like /proposals/123)
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <header className="h-16 border-b border-slate-200 bg-white md:hidden flex items-center px-4 justify-between">
           <div className="flex items-center gap-2">
               <MobileNav /> {/* <--- Hamburger Menu Here */}
               <span className="font-bold text-lg">FoundrKit</span>
           </div>
           <div className="h-8 w-8 rounded-full bg-slate-200"></div> {/* User Avatar Placeholder */}
        </header>
