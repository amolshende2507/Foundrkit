...
// Extracted Google Icon for cleaner JSX
const GoogleIcon = () => (
...
export default function AuthPage() {
  ...
  // Helper to cleanly switch between Login and Sign Up
  const toggleAuthMode = () => {
    setIsLogin(!isLogin);
    setError(null);
    setPassword(""); // Clear password on switch for better UX/Security
  };
  ...
  // ✅ MAIN AUTH UI
  return (
    <div className="w-full h-screen grid grid-cols-1 md:grid-cols-2 overflow-hidden bg-slate-50">
      
      {/* LEFT SIDE */}
      <div className="hidden md:flex flex-col justify-between bg-slate-900 p-10 text-white relative">
        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=2564&auto=format&fit=crop')] bg-cover opacity-25 backdrop-blur-sm" />
      ...
      <div className="flex items-center justify-center p-8">
        <div className="w-full max-w-sm space-y-6 bg-white p-8 rounded-2xl shadow-[0_20px_50px_-20px_rgba(0,0,0,0.2)] border border-slate-100">
          ...
          <form onSubmit={handleAuth}>
            {/* Using a fieldset elegantly disables all inputs inside when loading */}
            <fieldset disabled={isLoading} className="space-y-4">
              ...
              <Button
                type="submit"
                className="w-full h-12 rounded-xl bg-slate-900 hover:bg-slate-800 text-white tracking-wide transition-all"
              >
